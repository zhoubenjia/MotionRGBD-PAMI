...
def ShuffleMix(x, lam):
    ...
    # replace_idx = sorted(random.sample(range(0, x.size(2)), round((1. - lam)*x.size(2))))
    # x[:, :, replace_idx, :, :] = x_flipped[:, :, replace_idx, :, :]

    # length = x.size(2)
    # video_1 = torch.zeros(length)
    # video_ori = video_1.clone()
    # video_2 = torch.ones(length)
    # video_1[replace_idx] = video_2[replace_idx]

    # similarity = torch.dist(video_ori, video_1, p=2)
    # max_simil = torch.dist(video_ori, video_2, p=2)
    # lam = 1. - similarity / max_simil
...
def ShuffleMix_v1(x, lam):
    ...
    if v_len <= length-v_len:
        x[:, :, replace_idx, :, :] = x_flipped[:, :, replace_idx, :, :]
        # x[:, :, replace_idx, :, :].mul_(lam).add_(x_flipped[:, :, replace_idx, :, :].mul_(1. - lam))
    else:
        x_flipped[:, :, replace_idx, :, :] = x[:, :, replace_idx, :, :]
        # x_flipped[:, :, replace_idx, :, :].mul_(lam).add_(x[:, :, replace_idx, :, :].mul_(1. - lam))
...
def Visfeature(args, model, inputs, v_path=None, weight_softmax=None):
    # TSNE cluster
    if args.FusionNet:
