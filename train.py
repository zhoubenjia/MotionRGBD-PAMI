...
def get_args_parser():
    ...
    parser.add_argument('--FusionNet', default='', choices=['cs16', 'cs32', 'cs64', 'cv16', 'cv32', 'cv64'],
...
def main(args):
    ...
    for epoch in range(strat_epoch, args.epochs):
        ...
        args.epoch = epoch
        train_acc, train_obj, meter_dict_train = train_one_epoch(train_queue, model, model_ema, criterion, optimizer, epoch, local_rank, loss_scaler, device, mixup_fn, captuer)
...
def train_one_epoch(train_queue, model, model_ema, criterion, optimizer, epoch, local_rank, loss_scaler, device,
    ...
    for step, (inputs, heatmap, target, _) in enumerate(train_queue):
        ...
        images = inputs
        if args.FusionNet:
            # inputs = (inputs, target, inputs_aux, target_aux)
            with torch.no_grad():
...
def infer(valid_queue, model, criterion, local_rank, epoch, device, captuer):
    ...
    for step, (inputs, heatmap, target, v_path) in enumerate(valid_queue):
        ...
        if args.FusionNet:
            globals()['Fusion_loss'] = CE(output[0], target) + CE(output[1], target)
            Total_loss = Fusion_loss
